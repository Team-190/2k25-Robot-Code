...
public class CompositeCommands {
  ...
  public static final class V1_StackUpCompositeCommands {
     ...
     */
    public static final Command namicReefHeighsetDyt(ReefState height, ElevatorCSB elevator) {
      return Commands.sequence(
  ...
  public static final class V3_EpsilonCompositeCommands {
    ...
    public static final Command optimalAutoScoreCoralSequence(
      ...
      return Commands.sequence(
          ...
          superstructure
              ...
              .until(
                  () -> {
                    if (height.equals(ReefState.L4))
                      return superstructure.armBelowThreshold();
                    else
                      return false;
                  }));
    }
    ...
    private static final ScoreSide optimalSideBarge() {
      boolean facingPositive = MathUtil.inputModulus(
          RobotState.getRobotPoseField().getRotation().getRadians(), 0, 2 * Math.PI) >= 0
          && MathUtil.inputModulus(
              RobotState.getRobotPoseField().getRotation().getRadians(), 0, 2 * Math.PI) < Math.PI;

      if (RobotState.getRobotPoseField().getX() < FieldConstants.fieldLength / 2)
        return facingPositive ? ScoreSide.RIGHT : ScoreSide.LEFT;
      else
        return facingPositive ? ScoreSide.LEFT : ScoreSide.RIGHT;
    }
          ...
          superstructure.override(
              () -> {
                manipulator.setArmGoal(
                    frc.robot.subsystems.v3_Epsilon.superstructure.manipulator.V3_EpsilonManipulatorConstants.ManipulatorArmState.EMERGENCY_EJECT_ANGLE);
                manipulator.setRollerGoal(
                    frc.robot.subsystems.v3_Epsilon.superstructure.manipulator.V3_EpsilonManipulatorConstants.ManipulatorRollerState.L4_SCORE);
              }));
    }
