...
public class V3_PootSuperstructureEdges {
  ...
  public record Edge(V3_PootSuperstructureStates from, V3_PootSuperstructureStates to) {
    public Edge(V3_PootSuperstructureStates from, V3_PootSuperstructureStates to) {
      this.from = from;
      this.to = to;
    }
  ...
  public static void loadEdgesFromDot(
      String path,
      Graph<V3_PootSuperstructureStates, EdgeCommand> graph,
      ElevatorFSM elevator,
      V3_PootIntake intake,
      V3_PootManipulator manipulator) {

    DOTImporter<V3_PootSuperstructureStates, EdgeCommand> importer = new DOTImporter<>();

    // --- Fix 1: safer vertex factory ---
    importer.setVertexFactory(
        id -> {
          try {
            return V3_PootSuperstructureStates.valueOf(id);
    ...
    for (EdgeCommand e : graph.edgeSet()) {
      V3_PootSuperstructureStates from = graph.getEdgeSource(e);
      V3_PootSuperstructureStates to = graph.getEdgeTarget(e);
  ...
  private static Command getEdgeCommand(
      V3_PootSuperstructureStates from,
      V3_PootSuperstructureStates to,
      ElevatorFSM elevator,
      V3_PootIntake intake,
      V3_PootManipulator manipulator) {

    V3_PootSuperstructurePose pose = to.getPose();

    if (to.equals(V3_PootSuperstructureStates.BARGE_SCORE)) {
