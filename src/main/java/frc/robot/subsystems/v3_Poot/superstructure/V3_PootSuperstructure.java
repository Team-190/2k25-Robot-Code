...
import frc.robot.subsystems.v3_Poot.superstructure.V3_PootSuperstructureEdges.EdgeCommand;
import frc.robot.subsystems.v3_Poot.superstructure.V3_PootSuperstructureEdges.GamePieceEdge;
...
/**
 * The V3_PootSuperstructure class manages the coordinated movement and state transitions of the
 * robot's major subsystems including elevator, manipulator, and intake.
 */
public class V3_PootSuperstructure extends SubsystemBase {
  private final Graph<V3_PootSuperstructureStates, EdgeCommand> graph;
  private final ElevatorFSM elevator;
  private final V3_PootIntake intake;
  private final V3_PootManipulator manipulator;
   ...
   */
  @Getter private V3_PootSuperstructureStates previousState;
   ...
   */
  @Getter private V3_PootSuperstructureStates currentState;
   ...
   */
  @Getter private V3_PootSuperstructureStates nextState;
   ...
   */
  @Getter private V3_PootSuperstructureStates targetState;
  ...
  public V3_PootSuperstructure(
    ...
    previousState = null;
    currentState = V3_PootSuperstructureStates.START;
    nextState = null;

    targetState = V3_PootSuperstructureStates.START;

    // Initialize the graph
    graph = new DefaultDirectedGraph<>(EdgeCommand.class);

    for (V3_PootSuperstructureStates vertex : V3_PootSuperstructureStates.values()) {
      graph.addVertex(vertex);
    }

    // Add edges between states
    V3_PootSuperstructureEdges.addEdges(graph, elevator, intake, manipulator);
  }
  ...
  public void periodic() {
    if (currentState != null && !currentState.equals(V3_PootSuperstructureStates.OVERRIDE)) {
    ...
    double armHeight =
        -manipulator.getArmAngle().rotateBy(new Rotation2d(-Math.PI / 2)).getSin()
                * V3_PootManipulatorConstants.ARM_PARAMETERS.LENGTH_METERS()
   ...
   */
  private void setGoal(V3_PootSuperstructureStates goal) {
   ...
   */
  private Optional<V3_PootSuperstructureStates> bfs(
      V3_PootSuperstructureStates start, V3_PootSuperstructureStates goal) {
    Map<V3_PootSuperstructureStates, V3_PootSuperstructureStates> parents = new HashMap<>();
    Queue<V3_PootSuperstructureStates> queue = new LinkedList<>();
    queue.add(start);
    parents.put(start, null);
    while (!queue.isEmpty()) {
      V3_PootSuperstructureStates current = queue.poll();
      if (current == goal) break;
      for (EdgeCommand edge :
          graph.outgoingEdgesOf(current).stream()
              .filter(edge -> isEdgeAllowed(edge, goal))
              .toList()) {
        V3_PootSuperstructureStates neighbor = graph.getEdgeTarget(edge);
    ...
    V3_PootSuperstructureStates nextState = goal;
    while (!nextState.equals(start)) {
      V3_PootSuperstructureStates parent = parents.get(nextState);
   ...
   */
  private boolean isEdgeAllowed(EdgeCommand edge, V3_PootSuperstructureStates goal) {
    ...
    if (!RobotState.isHasAlgae()) {
      V3_PootSuperstructureStates from = graph.getEdgeSource(edge);
      V3_PootSuperstructureStates to = graph.getEdgeTarget(edge);

      boolean needsFlip = RobotState.getScoreSide().equals(ScoreSide.RIGHT);

      if (goal == V3_PootSuperstructureStates.STOW_UP && needsFlip) {
        if (to == V3_PootSuperstructureStates.STOW_UP
            && from != V3_PootSuperstructureStates.INVERSE_FLIP_UP) {
          return false;
        }
      }
  ...
  public void setAutoStart() {
    currentState = V3_PootSuperstructureStates.START;
    nextState = null;
    targetState = V3_PootSuperstructureStates.STOW_DOWN;
    if (edgeCommand != null) {
      edgeCommand.getCommand().cancel();
    }
  }
   ...
   */
  private V3_PootSuperstructureStates getElevatorPosition() {
    switch (RobotState.getOIData().currentReefHeight()) {
      case STOW, CORAL_INTAKE -> {
        return V3_PootSuperstructureStates.STOW_DOWN;
      }
      case L1 -> {
        return V3_PootSuperstructureStates.L1;
      }
      case L2 -> {
        return V3_PootSuperstructureStates.L2;
      }
      case L3 -> {
        return V3_PootSuperstructureStates.L3;
      }
      case L4 -> {
        return V3_PootSuperstructureStates.L4;
      }
      case ALGAE_INTAKE_BOTTOM -> {
        return V3_PootSuperstructureStates.L2_ALGAE;
      }
      case ALGAE_INTAKE_TOP -> {
        return V3_PootSuperstructureStates.L3_ALGAE;
      }
      case ALGAE_SCORE -> {
        return V3_PootSuperstructureStates.BARGE;
      }
      default -> {
        return V3_PootSuperstructureStates.START;
      }
    }
  }
   ...
   */
  public Command runGoal(V3_PootSuperstructureStates goal) {
   ...
   * @return Command to run the goal
   */
  public Command runGoal(Supplier<V3_PootSuperstructureStates> goal) {
    ...
    var tolerance =
        Optional.of(
            Rotation2d.fromRadians(
                V3_PootManipulatorConstants.CONSTRAINTS.goalToleranceRadians().get()));
  ...
  public Command override(Runnable action) {
    return Commands.sequence(runGoal(V3_PootSuperstructureStates.OVERRIDE), Commands.run(action))
   ...
   */
  public Command runGoalUntil(V3_PootSuperstructureStates goal, BooleanSupplier condition) {
    return Commands.sequence(runGoal(goal), Commands.waitUntil(condition));
  }

  public Command runGoalUntil(
      Supplier<V3_PootSuperstructureStates> goal, BooleanSupplier condition) {
    return Commands.sequence(runGoal(goal), Commands.waitUntil(condition));
  }
  ...
  public Command runReefScoreGoal(Supplier<ReefState> goal) {
    ...
    return this.runGoal(
        () -> {
          switch (goal.get()) {
            case L1:
              return V3_PootSuperstructureStates.L1_SCORE;
            case L2:
              return V3_PootSuperstructureStates.L2_SCORE;
            case L3:
              return V3_PootSuperstructureStates.L3_SCORE;
            case L4:
              return V3_PootSuperstructureStates.L4_SCORE;
            default:
              return V3_PootSuperstructureStates.STOW_DOWN;
          }
        });
    ...
    //     () ->
    //         switch (goal.get()) {
    //           case L1 -> V3_PootSuperstructureStates.L1;
    //           case L2 -> V3_PootSuperstructureStates.L2;
    //           case L3 -> V3_PootSuperstructureStates.L3;
    //           case L4 -> V3_PootSuperstructureStates.L4;
    //           default -> V3_PootSuperstructureStates.STOW_DOWN;
    //         },
    //     () ->
    //         switch (goal.get()) {
    //           case L1 -> 0.8;
  ...
  public Command runActionWithTimeout(
      Supplier<V3_PootSuperstructureStates> pose,
      Supplier<V3_PootSuperstructureStates> action,
   ...
   */
  public Command runActionWithTimeout(
      V3_PootSuperstructureStates pose, V3_PootSuperstructureStates action, double timeout) {
    return Commands.sequence(
            runGoal(action), // Run the action
            waitUntilAtGoal(),
   ...
   */
  public Command runActionWithTimeout(
      Supplier<V3_PootSuperstructureStates> action, DoubleSupplier timeout) {
    // Maps each action state to its corresponding pose state
    Map<V3_PootSuperstructureStates, V3_PootSuperstructureStates> actionPoseMap =
        new HashMap<>() {
          {
            put(V3_PootSuperstructureStates.L1_SCORE, V3_PootSuperstructureStates.L1);
            put(V3_PootSuperstructureStates.L2_SCORE, V3_PootSuperstructureStates.L2);
            put(V3_PootSuperstructureStates.L3_SCORE, V3_PootSuperstructureStates.L3);
            put(V3_PootSuperstructureStates.L4_SCORE, V3_PootSuperstructureStates.L4);
            put(V3_PootSuperstructureStates.BARGE_SCORE, V3_PootSuperstructureStates.BARGE);
            put(V3_PootSuperstructureStates.PROCESSOR_SCORE, V3_PootSuperstructureStates.PROCESSOR);
          }
        };

    // Reverse the map so we can look up the pose from action if needed
    Map<V3_PootSuperstructureStates, V3_PootSuperstructureStates> poseActionMap =
   ...
   * @return Command sequence to perform and recover from the action
   */
  public Command runActionWithTimeout(V3_PootSuperstructureStates action, double timeout) {
    // Maps each action state to its corresponding pose state
    Map<V3_PootSuperstructureStates, V3_PootSuperstructureStates> actionPoseMap =
        new HashMap<>() {
          {
  ...
  public Command allTransition() {
    Command all = runGoal(V3_PootSuperstructureStates.STOW_DOWN);
    for (var source : V3_PootSuperstructureStates.values()) {
      for (var sink : V3_PootSuperstructureStates.values()) {
        if (source == sink) continue;
        if (sink == V3_PootSuperstructureStates.FLIP_DOWN) continue;
        if (sink == V3_PootSuperstructureStates.FLIP_UP) continue;
        if (sink == V3_PootSuperstructureStates.STOW_DOWN) continue;
        if (sink == V3_PootSuperstructureStates.STOW_UP) continue;
        if (sink == V3_PootSuperstructureStates.GROUND_ACQUISITION) continue;
        if (sink == V3_PootSuperstructureStates.GROUND_INTAKE_ALGAE) continue;
        if (sink == V3_PootSuperstructureStates.GROUND_AQUISITION_ALGAE) continue;
        if (sink == V3_PootSuperstructureStates.GROUND_INTAKE) continue;
        if (sink == V3_PootSuperstructureStates.L1) continue;
        if (sink == V3_PootSuperstructureStates.L1_SCORE) continue;
        if (sink == V3_PootSuperstructureStates.L2) continue;
        if (sink == V3_PootSuperstructureStates.L2_SCORE) continue;
        if (sink == V3_PootSuperstructureStates.L3) continue;
        if (sink == V3_PootSuperstructureStates.L3_SCORE) continue;

        if (source != V3_PootSuperstructureStates.START
            && sink != V3_PootSuperstructureStates.START
            && source != V3_PootSuperstructureStates.OVERRIDE
            && sink != V3_PootSuperstructureStates.OVERRIDE) {
          all =
              all.andThen(
                  runGoal(sink),
                  runOnce(
  ...
  public Command stateTransitions(V3_PootSuperstructureStates source) {
    Command all = Commands.none();

    for (var sink : V3_PootSuperstructureStates.values()) {
      if (sink == source
          || sink == V3_PootSuperstructureStates.START
          || sink == V3_PootSuperstructureStates.OVERRIDE) continue;
