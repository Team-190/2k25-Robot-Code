...
import frc.robot.subsystems.v3_Poot.superstructure.V3_PootSuperstructureAction.SubsystemActions;
import frc.robot.subsystems.v3_Poot.superstructure.V3_PootSuperstructurePose.SubsystemPoses;
import frc.robot.subsystems.v3_Poot.superstructure.intake.V3_PootIntakeConstants.IntakePivotState;
import frc.robot.subsystems.v3_Poot.superstructure.intake.V3_PootIntakeConstants.IntakeRollerState;
...
public enum V3_PootSuperstructureStates {
  ...
  BARGE(
      "BARGE",
      new SubsystemPoses(
          ReefState.ALGAE_SCORE, ManipulatorArmState.PRE_SCORE, IntakePivotState.HANDOFF),
      SubsystemActions.empty()),
  BARGE_SCORE(
      "BARGE_SCORE",
      new SubsystemPoses(
  ...
  @Getter private final V3_PootSuperstructurePose pose;
  @Getter private final V3_PootSuperstructureAction action;
  ...
  V3_PootSuperstructureStates(String name, SubsystemPoses pose, SubsystemActions action) {
    this.pose = new V3_PootSuperstructurePose(name, pose);
    this.action = new V3_PootSuperstructureAction(name, action);
  }

  V3_PootSuperstructureStates(
      String name, SubsystemPoses pose, SubsystemActions action, Double elevatorTolerance) {
    this.pose = new V3_PootSuperstructurePose(name, pose, elevatorTolerance);
    this.action = new V3_PootSuperstructureAction(name, action);
  }

  V3_PootSuperstructureStates(
      String name, SubsystemPoses pose, SubsystemActions action, Rotation2d armTolerance) {
    this.pose = new V3_PootSuperstructurePose(name, pose, armTolerance);
    this.action = new V3_PootSuperstructureAction(name, action);
  }
}
