...
public class V2_RedundancyIntakeIOSim implements V2_RedundancyIntakeIO {
  ...
  public void updateInputs(IntakeIOInputs inputs) {
    ...
    inputs.extensionPositionMeters = (extensionSim.getAngleRads());
    inputs.extensionVelocityMetersPerSecond = extensionSim.getVelocityRadPerSec();
    inputs.extensionAppliedVolts = extensionAppliedVolts;
    inputs.extensionSupplyCurrentAmps = extensionSim.getCurrentDrawAmps();
    inputs.extensionGoal = (extensionController.getGoal().position);
    inputs.extensionPositionSetpoint = (extensionController.getSetpoint().position);
    inputs.extensionPositionError = (extensionController.getPositionError());
