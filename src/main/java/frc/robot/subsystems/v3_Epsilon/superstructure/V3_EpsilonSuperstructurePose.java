...
public class V3_EpsilonSuperstructurePose {
  ...
  public Command asTargetSpaceCommand(
      ElevatorFSM elevator, V3_EpsilonIntake intake, V3_EpsilonManipulator manipulator) {
    return new TargetSpaceCommand(this, elevator, intake, manipulator);
  }

  public class TargetSpaceCommand extends Command {
    ...
    TargetSpaceCommand(
        V3_EpsilonSuperstructurePose pose,
        ElevatorFSM elevator,
        V3_EpsilonIntake intake,
        V3_EpsilonManipulator manipulator) {
      this.targetPose = forwardKinematics(pose.elevatorHeight, pose.armState);
    ...
    public void initialize() {
      ...
      currentPose = forwardKinematics(elevator, manipulator);
      distance = currentPose.getTranslation().getDistance(targetPose.getTranslation());
      time = distance / maximumVelocityMetersPerSecond;
    }
  ...
  /**
   * Linearly interpolates between two Pose2d values using a factor t between 0 and 1. The resulting
   * Pose2d is a weighted average of the startValue and endValue, where t represents the weight of
   * the endValue.
   *
   * @param startValue The starting value of the interpolation.
   * @param endValue The ending value of the interpolation.
   * @param t The interpolation factor between 0 and 1.
   * @return The interpolated Pose2d value.
   */
  public Pose2d lerp(Pose2d startValue, Pose2d endValue, double t) {
  ...
  /**
   * Computes the forward kinematics of the robot given the elevator and manipulator states. This
   * method takes into account the offset of the elevator from the robot's base, the position of the
  ...
  public static Pose2d forwardKinematics(ReefState elevatorHeight, ManipulatorArmState armAngle) {
    return new Pose2d()
        .plus(
            new Transform2d(
                0,
        ...
        .plus(
            new Transform2d(
                0.0,
                ElevatorConstants.REEF_STATE_ELEVATOR_POSITION_MAP
                    .get(elevatorHeight)
                    .getPosition(),
                new Rotation2d()))
        .plus(
            new Transform2d(
                0.0,
                0.0,
                armAngle.getAngle(RobotState.getScoreSide()).plus(new Rotation2d(Math.PI / 2))))
        .plus(
            new Transform2d(
                V3_EpsilonManipulatorConstants.ARM_PARAMETERS.LENGTH_METERS(),
                0.0,
  ...
  /**
   * Computes the inverse kinematics of the robot given a translation from the robot's base. This
   * method takes into account the offset of the elevator from the robot's base, the position of the
   * elevator, and the angle of the manipulator's arm.
   *
