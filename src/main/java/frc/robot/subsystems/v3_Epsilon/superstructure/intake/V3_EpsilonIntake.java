...
public class V3_EpsilonIntake {
  ...
  /**
   * Periodic function for intake subsystem.
   * Updates inputs from the intake, and then sets the goals for the pivot and roller motors.
   * If the intake is in closed-loop mode, it sets the pivot motor goal to the desired angle.
   * Otherwise, it does not set the pivot motor goal.
   * It always sets the inner and outer roller motor goals to the desired voltage.
   */
  ...
  /**
   * Checks if the intake is currently detecting coral.
   * This is done by checking if both the left and right CAN range
   * sensors are detecting a distance greater than the coral detection
   * threshold.
   * 
  ...
  /**
   * Checks if the pivot motor is at the goal angle.
   * This is done by checking if the absolute difference between the current
   * pivot motor angle and the goal angle is less than the goal tolerance.
   * 
...
/**
 * Sets the goal state of the pivot motor.
 * If the intake is currently in open-loop mode, this method will set the isClosedLoop flag to true.
 * It then sets the pivotGoal field to the specified goal state.
 * The pivot motor will then be controlled in closed-loop mode to reach the specified goal state.
 * 
...
/**
 * Waits until the pivot motor is at the goal angle.
 * This command is created by sequencing a wait command with a waitUntil command.
 * The wait command waits for 0.02 seconds, and the waitUntil command checks if the pivot motor is at the goal angle.
 * The command will block until the pivot motor is at the goal angle.
