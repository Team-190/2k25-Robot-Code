...
import frc.robot.subsystems.v2_Redundancy.superstructure.V2_RedundancySuperstructurePose;
...
public class V3_SuperstructurePoses {
    ...
    public Command setIntakeState(V3_EpsilonIntake intake) {
        Commands.parallel(Command.runOnce(() -> intake.setGoal(intakeState),
        () -> intake.atGoal()));
    }
    ...
    public Command asCommand(ElevatorFSM elevator, V3_EpsilonManipulator manipulator, V3_EpsilonIntake intake) {
        return Commands.parallel(
            setElevatorHeight(elevator),
