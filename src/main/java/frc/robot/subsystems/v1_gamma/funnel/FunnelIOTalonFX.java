...
public class FunnelIOTalonFX implements FunnelIO {
    ...
    private final Alert crabDisconnectedAlert = new Alert("Funnel Crab Motor Disconnected. Check CAN bus!", AlertType.ERROR);
    private final Alert intakeDisconnectedAlert = new Alert("Funnel Intake Motor Disconnected. Check CAN bus!", AlertType.ERROR);
    ...
    public FunnelIOTalonFX() {
        ...
        crabConfig.CurrentLimits.withSupplyCurrentLimit(FunnelConstants.CURRENT_LIMITS.CRAB_SUPPLY_CURRENT_LIMIT());
        crabConfig.CurrentLimits.withStatorCurrentLimit(FunnelConstants.CURRENT_LIMITS.CRAB_STATOR_CURRENT_LIMIT());
        ...
        crabConfig.SoftwareLimitSwitch.ForwardSoftLimitThreshold = FunnelConstants.ANGLE_THRESHOLDS.MAX_ANGLE_RADIANS().get();
        crabConfig.SoftwareLimitSwitch.ReverseSoftLimitThreshold = FunnelConstants.ANGLE_THRESHOLDS.MIN_ANGLE_RADIANS().get();
        crabConfig.MotionMagic.MotionMagicAcceleration = FunnelConstants.CRAB_MOTOR_CONSTRAINTS.MAX_ACCELERATION().get();
        crabConfig.MotionMagic.MotionMagicCruiseVelocity = FunnelConstants.CRAB_MOTOR_CONSTRAINTS.MAX_VELOCITY().get();

        TalonFXConfiguration intakeConfig = new TalonFXConfiguration();
        intakeConfig.CurrentLimits.withSupplyCurrentLimit(FunnelConstants.CURRENT_LIMITS.INTAKE_SUPPLY_CURRENT_LIMIT());
        intakeConfig.CurrentLimits.withStatorCurrentLimit(FunnelConstants.CURRENT_LIMITS.INTAKE_STATOR_CURRENT_LIMIT());
        ...
        intakeConfig.MotionMagic.MotionMagicAcceleration = FunnelConstants.INTAKE_MOTOR_CONSTRAINTS.MAX_ACCELERATION().get();
        intakeConfig.MotionMagic.MotionMagicCruiseVelocity = FunnelConstants.INTAKE_MOTOR_CONSTRAINTS.MAX_VELOCITY().get();

        CANcoderConfiguration canCoderConfig = new CANcoderConfiguration();
        canCoderConfig.MagnetSensor.MagnetOffset = FunnelConstants.CANCODER_ABSOLUTE_OFFSET_ROTATIONS.get();
    ...
    public void updateInputs(FunnelIOInputs inputs) {
        ...
        inputs.crabVelocityRadiansPerSecond = Units.rotationsToRadians(crabVelocityRotationsPerSecond.getValueAsDouble());
        ...
        inputs.encoderPositionRadians = Units.rotationsToRadians(encoderPositionRotations.getValueAsDouble());

        inputs.intakePositionRadians = Units.rotationsToRadians(intakePositionRotations.getValueAsDouble());
        inputs.intakeVelocityRadiansPerSecond = Units.rotationsToRadians(intakeVelocityRotationsPerSecond.getValueAsDouble());
        ...
        inputs.intakeVelocitySetpointRadiansPerSecond = intakeVelocitySetpointRotationsPerSecond.getValueAsDouble();
        inputs.intakeVelocityErrorRadiansPerSecond = intakeVelocityErrorRotationsPerSecond.getValueAsDouble();
    ...
    public void setCrabPosition(double radians) {
        ...
        crabMotor.setControl(positionControlRequest.withPosition(Units.radiansToRotations(radians)).withEnableFOC(true));
    }

    @Override
    public void setIntakeVelocity(double radiansPerSecond) {
        intakeGoalRadiansPerSecond = radiansPerSecond;
        intakeMotor.setControl(velocityControlRequest.withVelocity(Units.radiansToRotations(radiansPerSecond)).withEnableFOC(true));
    }
    ...
    public boolean atCrabGoal() {
        return Math.abs(crabGoalRadians - Units.rotationsToRadians(crabPositionRotations.getValueAsDouble())) < FunnelConstants.CRAB_MOTOR_CONSTRAINTS.GOAL_TOLERANCE().get();
    }

    @Override
    public boolean atIntakeGoal() {
        return Math.abs(intakeGoalRadiansPerSecond - Units.rotationsToRadians(intakeVelocityRotationsPerSecond.getValueAsDouble())) < FunnelConstants.INTAKE_MOTOR_CONSTRAINTS.GOAL_TOLERANCE().get();
    }
}
